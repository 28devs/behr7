...
(function (global, factory) {
    ...
    niceDatePicker.prototype.buildUi = function (year, month) {
        ...
        if (!this.$params.mode) {
            ...
            for (var i = 0; i < this.dayWords[1].length; i++) {
                html += '<div class="calendar__day">' + this.dayWords[1][i] + '</div>';
            }
        } else if (this.$params.mode === 'zh') {
            for (var i = 0; i < this.dayWords[0].length; i++) {
                html += '<th>' + this.dayWords[0][i] + '</th>';
        ...
        for (var i = 0; i < this.monthData.date.length; i++) {
            if (i % 7 === 0) {
                html += '<div class="calendar__days-row">';
            }
            html += '<div class="calendar__days-col ' + this.monthData.date[i].styleCls + '" data-date="' + this.monthData.year + '/' + this.monthData.month + '/' + this.monthData.date[i].showDate + '">' + this.monthData.date[i].showDate + '</div>';
            if (i % 7 === 6) {
                html += '</div>';
            }
        }
    ...
    niceDatePicker.prototype.init = function ($params) {
        ...
        const plainEvents = $params.plainEvent
        plainEvents.forEach(function(item) {
          let elem = document.querySelector('[data-date="' + item + '"]')
          !elem || elem.classList.add('plain-event')
        })
        ...
        this.$warpper.addEventListener('click', function (e) {
            ...
            if ($target.classList.contains('nice-normal')) {
                ...
                $params.onClickDate($target.getAttribute('data-date'), $target);

                $params.onReady($target.getAttribute('data-date'), $target);
